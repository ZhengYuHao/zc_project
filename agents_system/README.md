...
系统支持记录每次模型调用的详细信息，并可将这些信息存储到飞书多维表格中。

### 工作原理

1. 每次模型调用时，相关信息（任务类型、提示词、模型响应、调用时间等）被记录在内存中
2. 所有记录都与当前请求的request_id关联
3. 当请求处理完成时，系统统一将该请求的所有模型调用记录写入飞书多维表格

### 配置

在 `config/model_config.json` 文件中添加以下配置：
...
- `enabled`: 是否启用模型调用日志记录功能
- `app_token`: 飞书多维表格的应用令牌
- `table_id`: 飞书多维表格的表格ID

### 功能说明

当启用模型调用日志记录功能后，每次调用模型时，系统会自动记录以下信息：
- 任务类型
- 提示词
- 模型响应
- 调用时间
...
这些信息会在请求处理完成后，统一异步写入到指定的飞书多维表格中，不影响主流程的性能。
